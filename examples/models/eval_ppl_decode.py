...
model_dir = "/data/baseline_ep2_lr1e-5_data_merge_1228_full_half.json1228123810_checkpoint-105_upload"
...
def test_llama_output():
    model_dir = "/project/djz/llama-13B-v1-0529"
...
def load_real_dataset():
    import json
    f = open("/workspace/EETQ/data_speed.json", "r")
    res = []
...
def ppl_eval():
    ...
    testdata = load_real_dataset()
    tokenizer = AutoTokenizer.from_pretrained(model_dir, trust_remote_code=True)
    config = AutoConfig.from_pretrained(model_dir, trust_remote_code=True)

    # baichuan
    
    ts_model = AutoModelForCausalLM.from_pretrained(model_dir, config=config, trust_remote_code=True).to(torch.float16)
    from eetq.utils import eet_accelerator
    eet_accelerator(ts_model, quantize=True, fused_attn=True, dev="cuda:0")
    ts_model.to("cuda:0")
    # 2. Chinese dataset
    ...
    for i in range(len(testdata)):
        s = testdata[i]
        idx = s.find("<|Assistant|>:\n")
        input_data.append(tokenizer(s[:idx + 16], return_tensors='pt').input_ids[0].unsqueeze(0).cuda())
        output_data.append(tokenizer(s[idx + 16:], return_tensors='pt').input_ids[0].unsqueeze(0).cuda())
        
    nlls = []
    loss_fct = torch.nn.CrossEntropyLoss(reduction="none")
    with torch.no_grad():
        for batch_idx in tqdm(range(len(input_data))):
            input_id = input_data[batch_idx]
            
            # output = ts_model(input_ids=input_id, past_key_values=None, attention_mask=None, use_cache=False)
            output = ts_model.generate(input_ids=input_id, max_length=input_id.shape[-1] + 4, output_scores=True, return_dict_in_generate=True, eos_token_id=-100)
            lm_logits = output.scores[-1]
            # Shift so that tokens < n predict n
            shift_logits = lm_logits.contiguous()
            shift_labels = output_data[batch_idx][..., 2].contiguous()
            # Flatten the tokens
    ...
    ppl = np.exp(np.mean([i.item() for i in nlls]))
    print("ppl: ", ppl)
    return ppl
