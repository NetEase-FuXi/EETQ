...
class Qwen2EETQForCausalLM(BaseEETQForCausalLM):
    ...
    def split_layers(self):
        if self.tp > 1:
            ...
            self.fuser.merge_tp()
        print("[EET][INFO] splitting qkv and gateup ...")
...
class Qwen2Fuser:
    ...
    def fuse_qkv_gateup(self):
        ...
        for name, m in self.model.named_modules():
            if isinstance(m, nn.Linear) and name != "lm_head":
                levels = name.split(".")
        ...
        for layer_idx, qkv in enumerate(all_qkv):
            self.qkv_index_map[layer_idx][1] = qkv[0].weight.shape[0]
            self.qkv_index_map[layer_idx][2] = self.qkv_index_map[layer_idx][1] + qkv[1].weight.shape[0]
            name = f"model.layers.{layer_idx}.self_attn.qkv_proj"
            qkv_weight = [x.weight for x in qkv]
            qkv_weight = torch.cat(qkv_weight, dim=0)
            fused_qkv = nn.Linear(qkv_weight.shape[1], qkv_weight.shape[0], bias=qkv[0].bias is not None)
            fused_qkv.weight = nn.Parameter(qkv_weight.to(device), requires_grad=False)
            if fused_qkv.bias is not None:
                fused_qkv.bias = nn.Parameter(torch.cat([x.bias for x in qkv if x.bias is not None]), requires_grad=False)
            replace_fused_qkv(self.model, name, fused_qkv)
        
        for layer_idx, gateup in enumerate(all_gateup):
            self.gateup_index_map[layer_idx][1] = gateup[0].weight.shape[0]
            name = f"model.layers.{layer_idx}.mlp.gateup_proj"
            gateup_weight = [x.weight for x in gateup]
            gateup_weight = torch.cat(gateup_weight, dim=0)
            fused_gateup = nn.Linear(gateup_weight.shape[1], gateup_weight.shape[0], bias=gateup[0].bias is not None)
            fused_gateup.weight = nn.Parameter(gateup_weight.to(device), requires_grad=False)
            if fused_gateup.bias is not None:
                fused_gateup.bias = nn.Parameter(torch.cat([x.bias for x in gateup if x.bias is not None]), requires_grad=False)
    ...
    def split_qkv_gateup(self):
        ...
        for name, m in modules:
            if isinstance(m, W8A16Linear) and name != "lm_head":
                levels = name.split(".")
                num_layers = int(levels[2])
                linear_name = levels[4]
    ...
    def split_tp(self, tp=2):
        ...
        for name, m in modules:
            if isinstance(m, nn.Linear) and name != "lm_head":
                levels = name.split(".")
                linear_name = levels[4]
                if linear_name in ["qkv_proj", "gateup_proj"]:
                    split_tp_column(self.model, name, m, tp)
                elif linear_name in ["o_proj", "down_proj"]:
